...
// LocalStorage Helper for Offline Session Backup
...
function saveLocalSession(sessionData: any) {
  ...
  const updated = [sessionData, ...existing.filter((s: any) => s.id !== sessionData.id)];
...
export async function fetchSessions(): Promise<WorkoutSession[]> {
  try {
    ...
    // Cache clean API sessions to local storage
    localStorage.setItem('mjolnir_sessions_backup', JSON.stringify(apiSessions));
    return apiSessions;
  } catch (err) {
    // If backend offline, return local backup
    return getLocalSessions() as any;
  }
}
